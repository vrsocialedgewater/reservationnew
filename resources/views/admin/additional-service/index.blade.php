            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 card-no-border">
                        <button class="btn btn-secondary mb-3" id="single-row-delete-btn" onclick="openModal()"><i class="fa fa-plus"></i> Add Upgrade</button>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('additional_service-created', () => {
            openToast('success', 'Upgrade has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('additional_service-updated', () => {
            openToast('success', 'Upgrade has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('additional_service-update-error', () => {
            openToast('error', 'Upgrade update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('additional_service-deleted', () => {
            openToast('success', 'Upgrade has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/upgrades/'+id)
    }
