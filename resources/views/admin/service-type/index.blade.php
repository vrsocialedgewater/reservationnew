<div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 card-no-border">
                        <button class="btn btn-secondary mb-3" id="single-row-delete-btn" onclick="openModal()"><i class="fa fa-plus"></i> Add Experience Type</button>
                    </div>
                    <div class="card-body">
    ...
    <div wire:ignore.self class="modal fade" id="form_modal" tabindex="-1" role="dialog" aria-labelledby="form_modal" aria-hidden="true">
        <livewire:admin.service_type.form />
    </div>
</div>
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('service_type-created', () => {
            openToast('success', 'Experience type has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-updated', () => {
            openToast('success', 'Experience type has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-update-error', () => {
            openToast('error', 'Experience type update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_type-deleted', () => {
            openToast('success', 'Experience type has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/experience_types/'+id)
    }
