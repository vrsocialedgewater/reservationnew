            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 card-no-border">
                        <button class="btn btn-secondary mb-3" id="single-row-delete-btn" onclick="openModal()"><i class="fa fa-plus"></i> Add Experience</button>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('service-created', () => {
            openToast('success', 'Experience has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-updated', () => {
            openToast('success', 'Experience has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-update-error', () => {
            openToast('error', 'Experience update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service-deleted', () => {
            openToast('success', 'Experience has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/experiences/'+id)
    }
