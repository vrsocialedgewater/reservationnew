            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 card-no-border">
                        <button class="btn btn-secondary mb-3" id="single-row-delete-btn" onclick="openModal()"><i class="fa fa-plus"></i> Add Package</button>
                    </div>
                    <div class="card-body">
...
{{--                                                <button class="btn btn-outline-info" onclick="view({{ $item->id }})"><i class="icon-eye"></i></button>--}}
    ...
    <div wire:ignore.self class="modal fade" id="form_modal" tabindex="-1" role="dialog" aria-labelledby="form_modal" aria-hidden="true">
        <livewire:admin.service.package.form />
    </div>
</div>
<script>
    ...
    document.addEventListener('livewire:init', function () {
        ...
        Livewire.on('service_package-created', () => {
            openToast('success', 'Package has created!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_package-updated', () => {
            openToast('success', 'Package has updated!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_package-update-error', () => {
            openToast('error', 'Package update error. Please try again!')
            $('#form_modal').modal('hide');
        });

        Livewire.on('service_package-deleted', () => {
            openToast('success', 'Package has deleted!')
        });
    });
    ...
    function view(id) {
        Livewire.navigate('/admin/packages/'+id)
    }
